...
int main(int argc, char* argv[]) {
    ...
    for(int tt = 0; tt < iters / 2; tt++) {
      ...
      try {
        queue.submit([&] (sycl::handler& cgh) {
          ...
          local_accessor_t local_tot_u_acc(sycl::range<1>(LOCAL_NY * LOCAL_NX), cgh);
  ...
  ** scratch space grid */
  ...
  for (int offset = LOCAL_NX * LOCAL_NY / 2; offset > 0; offset >>= 1) {
   ...
   if (item_id == 0) {
     const int base = (2*tt) * (params.nx / LOCAL_NX) * (params.ny / LOCAL_NY);
     partial_tot_u_acc[base + g_ii + g_jj * (params.nx / LOCAL_NX)] = local_tot_u_acc[0];
      ...
      try {
        queue.submit([&] (sycl::handler& cgh) {
          ...
          auto tmp_speeds_0_acc = speeds_0_sycl.get_access<sycl::access::mode::discard_write>(cgh);
          auto tmp_speeds_1_acc = speeds_1_sycl.get_access<sycl::access::mode::discard_write>(cgh);
          auto tmp_speeds_2_acc = speeds_2_sycl.get_access<sycl::access::mode::discard_write>(cgh);
          auto tmp_speeds_3_acc = speeds_3_sycl.get_access<sycl::access::mode::discard_write>(cgh);
          auto tmp_speeds_4_acc = speeds_4_sycl.get_access<sycl::access::mode::discard_write>(cgh);
          auto tmp_speeds_5_acc = speeds_5_sycl.get_access<sycl::access::mode::discard_write>(cgh);
          auto tmp_speeds_6_acc = speeds_6_sycl.get_access<sycl::access::mode::discard_write>(cgh);
          auto tmp_speeds_7_acc = speeds_7_sycl.get_access<sycl::access::mode::discard_write>(cgh);
          auto tmp_speeds_8_acc = speeds_8_sycl.get_access<sycl::access::mode::discard_write>(cgh);
          auto speeds_0_acc = tmp_speeds_0_sycl.get_access<sycl::access::mode::read>(cgh);
          auto speeds_1_acc = tmp_speeds_1_sycl.get_access<sycl::access::mode::read>(cgh);
          auto speeds_2_acc = tmp_speeds_2_sycl.get_access<sycl::access::mode::read>(cgh);
          auto speeds_3_acc = tmp_speeds_3_sycl.get_access<sycl::access::mode::read>(cgh);
          auto speeds_4_acc = tmp_speeds_4_sycl.get_access<sycl::access::mode::read>(cgh);
          auto speeds_5_acc = tmp_speeds_5_sycl.get_access<sycl::access::mode::read>(cgh);
          auto speeds_6_acc = tmp_speeds_6_sycl.get_access<sycl::access::mode::read>(cgh);
          auto speeds_7_acc = tmp_speeds_7_sycl.get_access<sycl::access::mode::read>(cgh);
          auto speeds_8_acc = tmp_speeds_8_sycl.get_access<sycl::access::mode::read>(cgh);
          auto obstacles_acc = obstacles_sycl.get_access<sycl::access::mode::read>(cgh);
  ...
  ** scratch space grid */
  ...
  for (int offset = LOCAL_NX * LOCAL_NY / 2; offset > 0; offset >>= 1) {
   ...
   if (item_id == 0) {
     const int base = (2*tt+1) * (params.nx / LOCAL_NX) * (params.ny / LOCAL_NY);
     partial_tot_u_acc[base + g_ii + g_jj * (params.nx / LOCAL_NX)] = local_tot_u_acc[0];
   }
