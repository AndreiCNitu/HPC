...
void stencil2(const int nx, const int ny, double * image, double * tmp_image);
...
int main(int argc, char *argv[]) {
  ...
  double *saved_row = malloc(sizeof(double)*ny);
  ...
  for( int t = 0; t < niters * 2; t++ ) {
    stencil2(nx, ny, image, saved_row);
  }
...
void stencil2( const int nx, const int ny, double * image, double * tmp_row ) {
  double saved_current; 
  for( int i = 0; i < nx; i++ ) {
    for( int j = 0; j < ny; j++ ) {
      saved_current = image[ j + i * ny ];
      image[ j + i * ny ] *= 0.6;
